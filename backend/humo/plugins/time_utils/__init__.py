...
def get_current_datetime():
    ...
    """
    return arrow.now().format(DEFAULT_FORMAT)


def get_current_week_day() -> int:
    """
    description:  获取当前是周几
    :return:
    """
...
def get_last_week_start_date() -> str:
    ...
    return (
        arrow.now()
        .shift(weeks=-1, days=-get_current_week_day() + 1)
        .format("YYYY-MM-DD")
    )
...
def get_last_week_end_date() -> str:
    ...
    return (
        arrow.now()
        .shift(weeks=-1, days=-get_current_week_day() + 1 + 6)
        .format("YYYY-MM-DD")
    )
...
def get_birthday_date(age: str, date_time: str = get_current_date()):
    ...
    birthday = get_any_datetime(
        date_time=date_time, format="YYYY-MM-DD", **shift
    ).split(" ")[0]
    return birthday
...
def get_any_datetime(
    ...
    """
    return (
        arrow.get(date_time if date_time else get_current_datetime(), format)
        .shift(
            years=year,
            months=month,
            days=day,
...
def conduct_add_subtract_shift(shifts):
    shift = {"year": 0, "month": 0, "day": 0, "hour": 0, "minute": 0, "second": 0}
...
def get_last_any_day(last_day: int = 1, date_time=get_current_date()):
    return get_any_datetime(date_time=date_time, day=-last_day, format="YYYY-MM-DD")
...
def cal_time(date1, date2):
    date1 = arrow.get(date1, DEFAULT_FORMAT)
